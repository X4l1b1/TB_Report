...
Those algorithms and a graphical example of their execution are provided below. \\

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.5]{Figures/WL.png}
    \caption{Illustration of the Walk-Left Algorithm on $acaacg$}
    \label{fig:walkleft}
\end{figure}
...
\vspace*{5mm}
Once a final $[top,bot]$ interval is obtained, and given it is not empty, it is then required to revert the BWT in order to determine the position of the queried string in the original text. To that end, the \textsl{LF-Mapping} algorithm introduced in the previous section shows is used in the definition of another algorithm, \textsl{Walk-Left}. This procedure, presented below, can be used to fully revert the transform, given $Occ$, the transform $T^{bwt}$ and the FM-Index $F$, as described in the algorithm below and a graphic illustration is given in Figure \ref{fig:walkleft}.
